...
static func search_res(p_regex := "") -> Dictionary:
	var regex: RegEx
	if p_regex:
		regex = RegEx.new()
...
static func build_file_tree_dict(p_sort := SORT_SCENE_INHERITANCE) -> Dictionary:
	...
	var this := preload("res_utility.gd")
	assert(this)
	var temp_files := this.search_res(REGEX[p_sort]) as Dictionary
	var inserted_map := {}

	for a_filename in temp_files:
		var file_path := temp_files[a_filename]["path"] as String
		...
		var section: Variant = file_hierarchy.front()
